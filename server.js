...
console.log("about to call login");
        ...
        if(request.session.user) {
            ...
            if (request.files.file.size > 0) {
                ...
                fileSystem.readFile(request.files.file.path, function(err, data) {
                    if (err) throw err;console.log('File read!');
                   ...
                   fileSystem.readFile(request.files.file.path, function (err, data) {
                    ...
                    //write the file
                    fileSystem.writeFile(uploadedObj.filePath, data, async function (err,data) {
                        if (err) throw err;
                        console.log('File written!');
                ...
                request.status = "error";request.message = "Please select valid image.";
    ...
    app.post("/Register", async function (request, result) {
        ...
        if (user == null) {
            bcrypt.hash(password, 10, async function(error, hash) {
                await mongo.db("inshare").collection("users").insertOne({
                    "name": name,
                    "email": email,
                    "password": hash,
                    ...
                    var text = "Please verify your account by clicking on the following link: " + mainURL + "/verifyEmail/" + email + "/" + verification_token;

                    var html = "Please verify your account by clicking on the following link: <br><br> <a href='" + mainURL + "/verifyEmail/" + email + "/" + verification_token + "'>Confirm Email</a> <br><br> Thank you.";
                    ...
                    await transporter.sendMail({
                        ...
                        request.message = "Signed up successfully. An email has been sent to verify yoyr account.";
    ...
    app.get("/verifyEmail/:email/:verification_token", async function(request, result) {
        ...
        if (user == null) {
            ...
            await mongo.db("inshare").collection("users").findOneAndUpdate({
                $and: [{
                    "email": email,
                }, {
                    "verification_token": parseInt(verification_token)
    ...
    app.post("/Login", async function(request, result){
        ...
        app.post("/SendRecoveryLink", async function(request, result){
            ...
            transporter.sendMail({
                ...
                request.status = "success";
                request.message = "Email has been sent with the link to recover your password.";
